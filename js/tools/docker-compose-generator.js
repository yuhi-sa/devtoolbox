    ...
    function addService(tpl) {
      ...
      card.innerHTML =
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.75rem;">' +
          '<strong>サービス #' + id + '</strong>' +
          '<button type="button" class="btn btn--secondary" data-remove="' + id + '" style="padding:.25rem .5rem;font-size:.85rem;">削除</button>' +
        '</div>' +
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;">' +
          '<div><label class="label">サービス名</label><input type="text" class="input" data-field="name" value="' + escapeAttr(name) + '"></div>' +
          '<div><label class="label">イメージ</label><input type="text" class="input" data-field="image" value="' + escapeAttr(t.image) + '" placeholder="例: nginx:alpine"></div>' +
        '</div>' +
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;margin-top:.75rem;">' +
          '<div><label class="label">ポート（1行に1つ）</label><textarea class="textarea" data-field="ports" rows="2" placeholder="8080:80">' + escapeHtml(t.ports) + '</textarea></div>' +
          '<div><label class="label">ボリューム（1行に1つ）</label><textarea class="textarea" data-field="volumes" rows="2" placeholder="./data:/data">' + escapeHtml(t.volumes) + '</textarea></div>' +
        '</div>' +
        '<div style="margin-top:.75rem;"><label class="label">環境変数（1行に1つ、KEY=VALUE）</label><textarea class="textarea" data-field="env" rows="3" placeholder="KEY=value">' + escapeHtml(t.env) + '</textarea></div>' +
        '<div style="margin-top:.75rem;"><label class="label">depends_on（カンマ区切り）</label><input type="text" class="input" data-field="depends" value="' + escapeAttr(t.depends) + '" placeholder="例: db, cache"></div>';

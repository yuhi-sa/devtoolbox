  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function parseYAML(text) {
      ...
      for (var i = 0; i < lines.length; i++) {
        if (/\t/.test(lines[i])) {
          throw new YAMLError("タブ文字が検出されました。YAMLではインデントにスペースのみ使用できます。", i + 1);
        }
      }
      var ctx = { lines: lines, pos: 0 };
      var result = parseDocument(ctx);
      return result;
    }
    ...
    function parseDocument(ctx) {
      ...
      var stripped = line.replace(/#.*$/, "").trimEnd();
      ...
      if (ctx.pos < ctx.lines.length) {
        var endLine = ctx.lines[ctx.pos].trim();
        if (endLine === "..." || endLine === "---") {
          ctx.pos++;
        }
      }
    ...
    function parseValue(ctx, expectedIndent) {
      ...
      // Mapping
      if (contentForCheck.match(/^[^\s].*?:\s/) || contentForCheck.match(/^[^\s].*?:$/) ||
          contentForCheck.match(/^".*?":\s/) || contentForCheck.match(/^".*?":$/) ||
          contentForCheck.match(/^'.*?':\s/) || contentForCheck.match(/^'.*?':$/)) {
        return parseMapping(ctx, indent);
      }

      // Flow sequence [...]
      if (contentForCheck.charAt(0) === "[") {
        ctx.pos++;
        return parseFlowSequence(contentForCheck);
      }

      // Flow mapping {...}
      if (contentForCheck.charAt(0) === "{") {
        ctx.pos++;
        return parseFlowMapping(contentForCheck);
      }

      // Scalar value
      ctx.pos++;
      return parseScalar(contentForCheck);
    }
    ...
    function removeInlineComment(str) {
      ...
      for (var i = 0; i < str.length; i++) {
        ...
        if (ch === "#" && !inSingle && !inDouble && i > 0 && str.charAt(i - 1) === " ") {
    ...
    function parseFlowSequence(str) {
      ...
      return items.map(function (item) {
        item = item.trim();
        if (item.charAt(0) === "[") return parseFlowSequence(item);
        if (item.charAt(0) === "{") return parseFlowMapping(item);
    ...
    function parseFlowMapping(str) {
      ...
      for (var i = 0; i < items.length; i++) {
        var pair = items[i].trim();
        var colonIdx = pair.indexOf(":");
        if (colonIdx === -1) throw new YAMLError("フローマッピングのキー:値が不正です", 0);
        ...
        if (val.charAt(0) === "[") obj[key] = parseFlowSequence(val);
        else if (val.charAt(0) === "{") obj[key] = parseFlowMapping(val);
    ...
    function parseMapping(ctx, baseIndent) {
      ...
      while (ctx.pos < ctx.lines.length) {
        ...
        if (indent > baseIndent) {
          throw new YAMLError("インデントが不正です。", ctx.pos + 1);
        }

        var trimmed = line.trim();
        var content = removeInlineComment(trimmed);
        ...
        if (colonMatch) {
          ...
          valueStr = colonMatch[2].trim();
        } else {
          key = colonMatchEmpty[1].trim();
          valueStr = "";
        }
        ...
        if (valueStr === "" || valueStr === "~" || valueStr === "null") {
          // Check if next line has deeper indent (nested value)
          skipBlankAndComments(ctx);
          if (ctx.pos < ctx.lines.length) {
            var nextIndent = getIndent(ctx.lines[ctx.pos]);
            var nextTrimmed = ctx.lines[ctx.pos].trim();
            if (nextIndent > baseIndent && nextTrimmed !== "") {
              // Check for block scalar or nested structure
              obj[key] = parseValue(ctx, nextIndent);
              continue;
            }
          }
          ...
          obj[key] = parseFlowSequence(valueStr);
        } else if (valueStr.charAt(0) === "{") {
          obj[key] = parseFlowMapping(valueStr);
        } else {
    ...
    function parseArray(ctx, baseIndent) {
      ...
      while (ctx.pos < ctx.lines.length) {
        ...
        var after = content.replace(/^-\s?/, "");
        ctx.pos++;

        if (after === "" || after === "~" || after === "null") {
          // Check if next line is a nested structure
          skipBlankAndComments(ctx);
          if (ctx.pos < ctx.lines.length) {
            var nextIndent = getIndent(ctx.lines[ctx.pos]);
            if (nextIndent > baseIndent) {
              arr.push(parseValue(ctx, nextIndent));
              continue;
            }
          }
          ...
          var tempCtx = { lines: [after], pos: 0 };
          var inlineObj = parseMapping(tempCtx, 0);
          // Check for continued nested keys at deeper indent
          skipBlankAndComments(ctx);
          if (ctx.pos < ctx.lines.length) {
            var ni = getIndent(ctx.lines[ctx.pos]);
            if (ni > baseIndent) {
              var nested = parseMapping(ctx, ni);
              for (var nk in nested) {
                if (nested.hasOwnProperty(nk)) inlineObj[nk] = nested[nk];
              }
            }
          }
          arr.push(inlineObj);
        } else if (after.charAt(0) === "[") {
          arr.push(parseFlowSequence(after));
        } else if (after.charAt(0) === "{") {
          arr.push(parseFlowMapping(after));
        } else if (after === "|" || after === ">" || after === "|-" || after === ">-" || after === "|+" || after === ">+") {
          arr.push(parseBlockScalar(ctx, baseIndent, after));
        } else {
    ...
    function parseBlockScalar(ctx, parentIndent, indicator) {
      var lines = [];
      var blockIndent = -1;
      var isLiteral = indicator.charAt(0) === "|";
      var chomp = indicator.length > 1 ? indicator.charAt(1) : "";
      ...
      while (ctx.pos < ctx.lines.length) {
        ...
        lines.push(line.substring(blockIndent));
        ctx.pos++;
      }

      // Remove trailing blank lines based on chomping
      if (chomp === "-") {
      ...
      if (isLiteral) {
        ...
        // Folded: join non-empty consecutive lines with space
        var parts = [];
        var current = "";
        for (var i = 0; i < lines.length; i++) {
          if (lines[i] === "") {
            ...
            current = current ? (current + " " + lines[i]) : lines[i];
          }
        }
        if (current) parts.push(current);
        result = parts.join("\n");
      ...
      if (chomp === "+") {
        ...
        // Default: single newline at end (clip), but we return without trailing newline for clean output
      }

      return result;
    }

    // Format parsed YAML back to string
    function formatYAML(data, indent) {
          ...
          if (val !== null && typeof val === "object" && !Array.isArray(val)) {
            ...
            if (keys.length > 0) {
              ...
              for (var j = 0; j < keys.length; j++) {
                var k = keys[j];
                var v = formatYAML(val[k], indent + 2);
                if (first) {
                  if (val[k] !== null && typeof val[k] === "object") {
                    lines.push(prefix + "- " + formatKey(k) + ":");
                    lines.push(v.split("\n").map(function (l) { return prefix + "    " + l; }).join("\n"));
                  } else {
                    lines.push(prefix + "- " + formatKey(k) + ": " + v);
                  }
                  first = false;
                } else {
                  if (val[k] !== null && typeof val[k] === "object") {
      ...
      if (typeof data === "object") {
        ...
        for (var oi = 0; oi < objectKeys.length; oi++) {
          var ok = objectKeys[oi];
          var ov = data[ok];
          if (ov !== null && typeof ov === "object") {
            objLines.push(prefix + formatKey(ok) + ":");
            var nested = formatYAML(ov, indent + 1);
            objLines.push(nested);
          } else {
            objLines.push(prefix + formatKey(ok) + ": " + formatYAML(ov, 0));
          }
        }
    ...
    btnValidate.addEventListener("click", function () {
      ...
      if (!input) {
        showError("YAMLを入力してください。");
        return;
      }
      try {
        parseYAML(input);
        showSuccess("有効なYAMLです。構文エラーはありません。");
        outputEl.value = "";
      } catch (e) {
        if (e.line) {
          showError("YAML構文エラー（" + e.line + "行目）: " + e.message);
        } else {
          showError("YAML構文エラー: " + e.message);
        }
        outputEl.value = "";
      }
    });
    ...
    btnFormat.addEventListener("click", function () {
      ...
      try {
        ...
        outputEl.value = formatYAML(parsed, 0);

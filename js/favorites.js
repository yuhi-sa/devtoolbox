...
(function () {
  ...
  function getSlugFromCard(card) {
    ...
    if (!href) return null;
    // Extract slug: "tools/json-formatter/" -> "json-formatter"
  ...
  function createStarButton(slug) {
    ...
    btn.addEventListener("click", function (e) {
      ...
      // Sync all star buttons for the same slug
  ...
  function addStarButtons() {
    ...
    cards.forEach(function (card) {
      ...
      if (!slug) return;
      // Don't add if already has a star
  ...
  function renderFavoritesSection() {
    ...
    // Find favorited cards from the main grid
    ...
    favCards.forEach(function (card) {
      // Clone the card for favorites section
      var clone = card.cloneNode(true);
      // Remove existing star button from clone and re-add
    ...
    // Re-attach star buttons to cloned cards in favorites section
  ...
  document.addEventListener("DOMContentLoaded", function () {
    // Only run on homepage (pages with tool-card elements in categories)
